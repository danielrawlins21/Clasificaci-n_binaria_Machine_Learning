...
Aquí quedan fijadas las reglas comunes del trabajo: partición estratificada, validación interna, selección de variables e hiperparámetros, ajuste del umbral y métricas de evaluación.
...
\begin{lstlisting}[caption={Protocolo resumido},label={lst:protocolo_resumido}]
cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
threshold, table = select_threshold(y_train, proba_cv, policy,
									np.array(default_cfg["threshold_grid"]))
\end{lstlisting}
...
La métrica primaria cambia según el objetivo del apartado; el resto de métricas se reportan para evitar conclusiones sesgadas por un único indicador.
